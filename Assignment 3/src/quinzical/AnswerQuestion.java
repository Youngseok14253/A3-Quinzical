...
/**
 * This AnswerQuestion Class provides a display method for the user to read, listen and answer the question. The window,
 * depending on the mode the user has selected, displays the question (either in text and/or via audio), allows the user 
 * to adjust the speed to of the audio, and submit an answer in the text-field given. This class is called by the Display 
 * classes when the user selects a category on the Board.
 * 
...
public class AnswerQuestion {
	...
	/*
	 * This method creates a new window, where it displays a question (either text/audio) from the Question Bank 
	 * corresponding to the selected category and waits for the user to submit an answer.  
	 * 
	 * @param categoryName The name of the Category in th Question Bank
	 * @param mode The mode (Game or Practice)
	 * @param categoryNumber The integer assigned to the Category when it is formatted
	 *
	 */
	public static void displayQuestion(String categoryName, String mode, int categoryNumber) {
		...
		//Pressing the "Answering Questions" button under the help menu will call displayHowToAnswer method,
		...
		//Calls the returnQuestionFormat method, which will return a line from the corresponding Category 
		...
		//Splits the String (Q/W/A) into a String Array, then back into its respectively named Strings
		...
		//This count is used to check the number of user attempts when answering a Question
		//If in Practice Mode, and the count becomes 2, then the hint is shown
		CompareAnswer.setCount();
		
		//If the submit button is pressed, then it passes the user input and answer into the compareAnswerToInput
		// method, and stores the boolean value output
		submitButton.setOnAction(e -> {
			boolean ans= CompareAnswer.compareAnswerToInput(answerField, answer);
			
			//If the answer is true, and in Game Mode, then it displays "Correct", and increments 
			...
			if (ans == true) {
				...
				//If the answer is false, and in Game Mode, then it displays "Incorrect", and exits
		...
		//If the user tries to close the window, a warning message will appear asking them if they are sure
